...
async function handleLogin(input, kv, request) {
    ...
    // 复用账号已有的持久 token（不再每次登录都重新生成）。
    // 旧逻辑每次登录都覆盖 token，会导致：
    //   1) 同账号在另一台设备登录时，把前一台设备的 token 顶失效 → “自动掉线”；
    //   2) KV 读改写竞态 + 边缘缓存使两次登录 token 不一致 → “不能二次登录”。
    // 改为持久 token 后，同账号可多设备/多次登录并存，且 token 不随缓存新旧变化。
    let token = accounts[username].token;
    if (!token) {
        token = generateToken();
        accounts[username].token = token;
    }

    // 更新登录信息（best-effort，即使写入失败也不影响本次登录鉴权）
    ...
    try {
        ...
        console.error('保存登录信息失败（已忽略）:', e && e.message);
    }
